...
An elementary correspondence from $F$ to $Y_F$ is associated to a closed point on $Y$. Since $Y_F$ is smooth over $Y$, the closed point is locally determined by a regular sequence of length equal to dimension of $Y$. Additionally our general setting requires that $Y$ is Noetherian, or at least that closed point generalizes to finitely many general points, so we may assume that $Y$ is affine. Then the ring of global sections of $Y_F$ is the colimit of that of $Y\times_kU$.
